...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		if slug and self.is_polylang():
			while self.check_slug_exist(slug):
				slug = to_str(slug) + '_' + to_str(language_code)
			...
			if self._notice['target']['support']['addons'] and not self.is_woo2woo():
				...
				for option in option_list:
					...
					for value in option['values']:
						...
						value_options_addons['price'] = value['option_value_price']
				...
				if children_list and to_len(children_list) <= self.VARIANT_LIMIT:
					...
					for key_child, product_child in enumerate(children_list):
						...
						self.insert_map(self.TYPE_apCHILD, product_child['id'], children_id, product_child['code'])
						...
						children_meta = {
							...
							'_stock': product_child['qty'] if product_child['qty'] else 0,
							'_regular_price': product_child['price'],
							'_sale_price': product_child['special_price']['price'] if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] == '0000-00-00' or product_child['special_price']['end_date'] == '0000-00-00 00:00:00' or product_child['special_price']['end_date'] == '' or convert['special_price']['end_date'] == None)) else '',
							'_price': product_child['special_price']['price'] if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] == '0000-00-00' or product_child['special_price']['end_date'] == '0000-00-00 00:00:00' or product_child['special_price']['end_date'] == '' or convert['special_price']['end_date'] == None)) else product_child['price'],
							'_download_limit': "",
							'_download_expiry': "",
							'_downloadable_files': "",
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		capabilities = php_serialize({'customer': True})
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		order_meta = {
			...
			'_order_currency': get_value_by_key_in_dict(convert, 'currency', 'USD'),
